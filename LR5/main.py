...
p.plot_error("jet", "error.png", delta2, u, "error.")
p.plot_error("jet", "error_scipy.png", delta2, u_sc, "scipy_error.")
p.plot_diff_my_scipy("jet", "diff_my_scipy.png", u, u_sc)

print("max_diff_with_scipy: ", np.max(np.abs(u - u_sc)))
xv, yv = np.meshgrid(t.xaxis, t.yaxis)
exact = t.exact(xv, yv, delta2)
print("max_scipy_error: ", np.max(np.abs(u_sc - exact)))
print("max_error: ", np.max(np.abs(u - exact)))
